---
title: "CSN -- Sensitivity analyses"
...
## Bias at cutoff score 60, 75
```{r}
for(co in c(60, 75)){
  base_rate <- predictions.t %>%
    filter(cutoff == co, base != "minority") %>%
    filter(identity == "Dutch") %>%
    select(predicted) %>%
    pull()
  
  predictions.t %>%
    filter(cutoff == co, base != "minority") %>%
    mutate(dhp = predicted/base_rate,
           dhp_l = conf.low/base_rate, 
           dhp_u = conf.high/base_rate) %>%
    mutate(across(starts_with("dhp"), ~fround(., 4))) %>%
    print()
}
```
